...
class DA_classifier:
    ...
    def get_annotated_transcript(self, fname, force_rebuild=False):
        ...
        PARAMS:
            ...
            Optional:
            bool force_rebuild: rebuilds from scratch even if tdf
                                already exists (default = False)
        RETURNS:
            pd.DataFrame tdf: annotated transcript
        """
        # load from config
        max_nr_utterances = config.data["max_nr_utterances"]

        transcript_dir = config.paths["transcripts"]
        df_dir = config.paths["tdfs"]
        ...
        if not os.path.exists(df_path) or force_rebuild:
            transcript = load_one_transcript(
                transcript_path, chunked=True, chunk_size=max_nr_utterances
            )
            tdf = self.make_annotated_transcript(transcript)
            tdf.to_csv(df_path, index=False)
        else:
            tdf = pd.read_csv(df_path)
    ...
    def make_annotated_transcript(self, transcript, verbose=False, model=None):
        ...
        X = make_model_readable_X(
            transcript_text, self.tokenizer, self.max_nr_utterances, self.max_nr_words
        )
        ...
        y_hat = merge_offset_arrays(
            y_hat, y_hat_offset, step=self.max_nr_utterances // 2
        )
        ...
        tdf = pd.DataFrame.from_records(
            sum(transcript, []), columns=["utterance", "speaker", "timestamp"]
        )
